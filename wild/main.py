...
# ):
#     """
#     Fetch a random subset of n activation vectors from the given layer.
#     We'll do this by repeatedly drawing batches from the activation store
#     until we accumulate n examples (or slightly more), then slice.

#     Args:
#         activation_store: Activation store to draw tokens from.
#         transformer: Model to run to get activations.
#         layer (int): Which layer to extract from.
#         device (str): Device to run on.
#         n (int): Number of activation vectors to collect.

#     Returns:
#         torch.Tensor: Activation matrix of shape (n, d_model).
#     """
#     collected = []
#     batch_size = activation_store.batch_size  # Typically same as store_batch_size
#     while sum(x.shape[0] for x in collected) < n:
...
#             collected.append(X.cpu())  # move to CPU to avoid large GPU usage
#             del cache, X, batch_tokens
#     # Concatenate
#     all_x = torch.cat(collected, dim=0)
#     # Slice exactly n
#     return all_x[:n].to(device)
...
# ):
#     """
#     Resample dead neurons according to the procedure:

#       1. Compute the loss on a random subset of 819,200 inputs.
#       2. Assign each input vector a probability proportional to the square 
#          of the autoencoder’s loss on that input.
#       3. For each dead neuron, sample an input according to these probabilities, 
#          renormalize, and set it to be the dictionary vector for that neuron.
#       4. For the corresponding encoder vector, renormalize it to be 
#          (average norm of alive neurons) * 0.2. 
#          Set the corresponding encoder bias element to 0.
...
#         activation_store: Activation store object to fetch a random subset from.
#         transformer (HookedTransformer): The transformer model.
#         device (str): Device used for training.
#         layer (int): Which layer to get activations from.
...
#     # 1. Get random subset of activation vectors (819,200 by default).
#     n_samples = 819200
#     X = get_random_activations(activation_store, transformer, layer, device, n=n_samples)

#     # 2. Compute autoencoder’s recon loss for each input, then use squared loss as probability.
#     with torch.no_grad():
#         S_, X_ = model(X)
#         # Recon loss per example is MSE per row (not normalized by dimension)
#         # We'll do sum across features, resulting in a scalar per row.
#         # (X_ - X)^2 => shape [n_samples, d_model]
#         recon_err = (X_ - X).pow(2).sum(dim=1)
#         # Use squared recon_err as probability
#         p = recon_err.pow(2)
#         p_sum = p.sum()
#         if p_sum < 1e-20:
#             # If the sum is extremely small, no strong signal to resample
#             # This can happen if the model is already near perfect on these samples
#             print("All reconstruction errors are near zero; skipping resampling.")
#             return
#         p /= p_sum

#     # We need the average norm of the alive neurons' encoder rows.
#     # We'll define 'alive' as ~dead_neurons.
#     alive_neurons = ~dead_neurons
#     with torch.no_grad():
#         # Extract the encoder and decoder weights
#         encoder_weight = model.encoder[0].weight.data  # shape [hidden_dim, input_dim]
#         decoder_weight = model.decoder.weight.data     # shape [input_dim, hidden_dim]

#         # For the average norm calculation, use only alive neurons
#         if alive_neurons.any():
#             alive_norms = torch.norm(encoder_weight[alive_neurons], dim=1)
#             avg_alive_norm = alive_norms.mean()
#         else:
#             # If everything is dead (unlikely, but let's be safe), pick a fallback
#             avg_alive_norm = 1.0

#     # 3-5. For each dead neuron, sample an input, reinit weights & reset Adam states.
#     num_resampled = 0
#     for idx in torch.where(dead_neurons)[0]:  # the indices of dead neurons
#         # 3. Sample an input from distribution p
#         sample_idx = torch.multinomial(p, 1).item()
#         # The chosen input vector
#         chosen_x = X[sample_idx].detach().clone()

#         # Renormalize chosen_x to unit L2 norm
#         norm_chosen_x = chosen_x.norm()
#         if norm_chosen_x > 1e-12:
#             chosen_x /= norm_chosen_x

#         # 4a. Set this as the dictionary vector (decoder weight column)
#         #     decoder_weight has shape [input_dim, hidden_dim], so the column is decoder_weight[:, idx].
#         decoder_weight[:, idx] = chosen_x

#         # 4b. For the corresponding encoder vector, renormalize to (avg_alive_norm * 0.2)
#         #     encoder_weight has shape [hidden_dim, input_dim], so the row is encoder_weight[idx].
#         #     We'll pick a direction the same as chosen_x, but scaled to that norm. 
#         #     Or simply reinit the row to random?  According to text, we "set the dictionary vector 
#         #     for the dead autoencoder neuron," and "renormalize the input vector for the encoder side." 
#         #     We'll do the approach described: if we want minimal interference, let's align it 
#         #     with chosen_x but scaled.
#         row_norm = encoder_weight[idx].norm()  # might be near zero
#         if chosen_x.norm() > 1e-12:
#             new_enc = chosen_x * (avg_alive_norm * 0.2 / chosen_x.norm())
#         else:
#             # fallback to Kaiming if chosen_x is somehow zero
#             new_enc = torch.nn.init.kaiming_uniform_(
#                 torch.empty_like(encoder_weight[idx]), a=math.sqrt(5)
#             )

#         encoder_weight[idx] = new_enc
#         # 4c. Set the corresponding encoder bias to zero
#         model.encoder[0].bias.data[idx] = 0.0

#         # 5. Reset the Adam optimizer parameters for these weights:
#         #    We must drop any accumulated momentum in the optimizer state.
#         #    This includes:
#         #        - encoder_weight[idx]
#         #        - decoder_weight[:, idx]
#         #        - encoder bias [idx]
#         params_to_reset = [
#             model.encoder[0].weight,  # entire param, but we only reset for row `idx`
#             model.decoder.weight,     # entire param, but only for column `idx`
#             model.encoder[0].bias     # entire param, but only for element `idx`
#         ]
#         for param in params_to_reset:
#             if param in optimizer.state:
#                 # Each param has a state dict keyed by e.g. param, not sub-tensors
#                 # So we must reset the entire param. This is a typical approach 
#                 # (fine-grained row resets are more complex in PyTorch).
...
#     upload_interval = 1000  # Upload every 1000 batches

#     # We also need to track neuron firing for the last 12,500 steps
#     # so we can detect dead neurons (neurons that haven't fired at all).
#     hidden_dim = model.encoder[0].weight.shape[0]
#     max_track_steps = 12500
...
#         # Track neuron firing for detection of dead neurons
#         # We say a neuron "fired" on a batch if it has any positive activation
#         # across the entire batch.
#         # S_ shape: (batch_size * seq_len, hidden_dim)
#         # We'll define "fired" as "any activation > 0" for that neuron in this batch.
#         fired = (S_ > 0).any(dim=0).cpu()  # shape [hidden_dim]
...
#         # Neuron resampling at steps 25k, 50k, 75k, and 100k
#         # (You can adjust or add more steps as desired.)
#         if batch_num in [25000, 50000, 75000, 100000]:
#             print(f"Performing neuron resampling at batch {batch_num}...")
#             # Identify dead neurons: those that have not fired (fired == 0) 
#             # in ANY of the last 12,500 steps.
...
#             # Clear the fired_buffer for the next cycle after resampling
#             fired_buffer.zero_()
...
#     hf_token = os.getenv("HF_TOKEN")  # Alternatively, use config_yml['HF_TOKEN']
